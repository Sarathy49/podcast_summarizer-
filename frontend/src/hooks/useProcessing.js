...
export const useProcessing = () => {
  ...
  const handleProcessing = async (endpoint, data, headers = {}) => {
    ...
    try {
      ...
      if (error.message === 'canceled' || error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        errorMessage = 'Request was cancelled - processing took too long. Try with a shorter audio file (5-10 minutes). For YouTube videos, use timestamps in URL to get a specific section.';
      } else if (error.response) {
        // Server returned an error response
        errorMessage = error.response.data?.detail || error.response.data?.error || error.message || 'Server error';
  ...
  const formatResult = (resultData) => {
    ...
    if (!formattedResult.transcript || formattedResult.transcript.includes("Error:")) {
      formattedResult.transcript = formattedResult.transcript || "Transcription failed. Please try again.";
      
      // If the transcript has error but we don't have a global error set, set it
      if (formattedResult.transcript.includes("Error:") && !formattedResult.error) {
        formattedResult.error = formattedResult.transcript;
      }
    }
    
    // Ensure segments is an array
    if (!Array.isArray(formattedResult.segments)) {
      formattedResult.segments = [];
    }
    
    // Ensure topics is an array
    if (!Array.isArray(formattedResult.topics)) {
      formattedResult.topics = [];
    }
    
    // Ensure extractive_summary is an array
    if (!Array.isArray(formattedResult.extractive_summary)) {
      formattedResult.extractive_summary = [];
    }
    
    // Ensure abstractive_summary is a string
    if (typeof formattedResult.abstractive_summary !== 'string') {
      formattedResult.abstractive_summary = formattedResult.abstractive_summary 
        ? String(formattedResult.abstractive_summary) 
        : "";
    }
    
    // Ensure metadata exists
    if (!formattedResult.metadata || typeof formattedResult.metadata !== 'object') {
