...
class TopicDetector:
    ...
    def _init_models(self):
        ...
        try:
            ...
            # If no model has been loaded yet, try with HF token
            if self._embedding_model is None:
                # Get HF token from environment variables
                token = os.environ.get("HF_ACCESS_TOKEN", os.environ.get("TRANSFORMERS_TOKEN", ""))
                
                if token:
                    try:
                        logger.info(f"Trying to load model with auth token: {self.embedding_model_name}")
                        self._embedding_model = SentenceTransformer(
                            self.embedding_model_name,
                            token=token
                        )
                        logger.info(f"Successfully loaded model with auth token: {self.embedding_model_name}")
                    except Exception as e:
                        logger.warning(f"Failed to load model with auth token: {str(e)}")
                        # Last attempt without token
                        self._embedding_model = SentenceTransformer("all-MiniLM-L6-v2")
                else:
                    logger.warning("No HF token found, using default model")
                    self._embedding_model = SentenceTransformer("all-MiniLM-L6-v2")
    ...
    def _fallback_keyword_extraction(self, text: str) -> List[Dict]:
        """
        Basic keyword extraction when BERTopic is not available.
        ...
        try:
            if NLTK_AVAILABLE:
                ...
                stop_words = set(stopwords.words('english'))
                # Add podcast-specific stopwords
                stop_words.update(self._podcast_stopwords)
                
                # Tokenize and remove stopwords
                words = nltk.word_tokenize(text.lower())
                words = [word for word in words if word.isalnum() and word not in stop_words and len(word) > 3]
                
                # Find bigrams (word pairs that occur together)
                bigram_measures = BigramAssocMeasures()
                finder = BigramCollocationFinder.from_words(words)
                finder.apply_freq_filter(3)  # Only consider pairs that appear at least 3 times
                bigrams = finder.nbest(bigram_measures.pmi, 10)  # Get top 10 collocations
                
                # Count word frequencies for single words
                from collections import Counter
                word_counts = Counter(words)
                
                # Get most common single words
                most_common = word_counts.most_common(20)
                
                # Combine single words and bigrams for better topic representation
                topics = []
                
                # Add bigrams first (they're usually more meaningful)
                for bg in bigrams:
                    bigram = ' '.join(bg)
                    topics.append((bigram, 3))  # Give bigrams a higher weight
                
                # Add single words
                for word, count in most_common:
                    if word not in [w for bg in bigrams for w in bg]:  # Don't include words already in bigrams
                        topics.append((word, count))
                
                # Sort by weight
                topics.sort(key=lambda x: x[1], reverse=True)
                
                # Group into 3-5 topics
                num_topics = min(5, max(3, len(topics) // 5))
                topics_per_group = max(1, len(topics) // num_topics)
                
                result = []
                for i in range(num_topics):
                    start_idx = i * topics_per_group
                    end_idx = start_idx + topics_per_group if i < num_topics - 1 else len(topics)
                    
                    topic_words = topics[start_idx:end_idx]
                    
                    if not topic_words:
                        continue
                        
                    # Calculate topic score based on frequency
                    total_weight = sum(weight for _, weight in topics)
                    topic_weight = sum(weight for _, weight in topic_words)
                    
                    # Create improved topic names
                    keywords = [word for word, _ in topic_words]
                    improved_keywords = self._improve_topic_names(keywords)
                    
                    topic_data = {
                        "id": i,
                        "keywords": keywords,
                        "improved_keywords": improved_keywords[:3],
                        "score": topic_weight / total_weight if total_weight > 0 else 0,
                        "sample_text": ""
                    }
                    
                    result.append(topic_data)
            else:
                # Fallback without NLTK - basic implementation
                # Define basic English stopwords
                basic_stopwords = {'a', 'an', 'the', 'and', 'or', 'but', 'if', 'because', 'as', 'what', 
                                  'when', 'where', 'how', 'who', 'which', 'this', 'that', 'these', 'those',
                                  'then', 'just', 'so', 'than', 'such', 'both', 'through', 'about', 'for',
                                  'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had',
                                  'do', 'does', 'did', 'to', 'at', 'in', 'on', 'by', 'from', 'with', 'of'}
                
                # Add podcast-specific stopwords
                all_stopwords = basic_stopwords.union(self._podcast_stopwords)
                ...
                # Group into 3-5 topics
                num_topics = min(5, max(3, len(most_common) // 5))
                words_per_topic = max(1, len(most_common) // num_topics)
                
                result = []
                for i in range(num_topics):
                    start_idx = i * words_per_topic
                    end_idx = start_idx + words_per_topic if i < num_topics - 1 else len(most_common)
                    
                    topic_words = most_common[start_idx:end_idx]
                    
                    if not topic_words:
                        continue
                        
                    total_count = sum(count for _, count in most_common)
                    topic_count = sum(count for _, count in topic_words)
                    
                    # Create improved topic names
                    keywords = [word for word, _ in topic_words]
                    improved_keywords = keywords[:2]  # Basic version just takes first two words
                    
                    topic_data = {
                        "id": i,
                        "keywords": keywords,
                        "improved_keywords": improved_keywords,
                        "score": topic_count / total_count if total_count > 0 else 0,
                        "sample_text": ""
                    }
                    
                    result.append(topic_data)
            
            logger.info(f"Extracted {len(result)} keyword groups using fallback method")
            return result
            
        except Exception as e:
            logger.error(f"Error during fallback keyword extraction: {str(e)}")
